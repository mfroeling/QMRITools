$(document).ready(function() {
    ...
    $(document).on('mouseenter', '.InCell', function() {
        ...
        setTimeoutConst = setTimeout(function(){
            inCell.find('.clipboard').after('<div class="tooltip">复制到剪切板</div>');
    ...
    $(document).on('mouseup', '.InCell', function(e){
        ...
        if (document.queryCommandSupported('copy')) {
            ...
            var text = '';
            var msg = successful ? text = '已复制' : text = 'Unable to copy.';
            $('.tooltip').text(text);
            $(this).find('.clipboard').addClass('copied');
            if($('.tooltip').length < 1) {
                clicked_element.find('.clipboard').after('<div class="tooltip">已复制</div>');
            }
        }
        else {
    ...
    $(document).on('touchend', '.InCell', function(e) {
        ...
        if (document.queryCommandSupported('copy')) {
            var successful = document.execCommand('copy');
            $(this).find('.IFL').after('<div class="clipboard"></div><div class="tooltip">已复制</div>');
            $(this).find('.clipboard').addClass('copied');
        }
        else {
            $(this).find('.IFL').addClass('show');
