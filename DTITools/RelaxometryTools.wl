...
Options[EPGT2Fit]= {
	DictB1Range -> {0.5, 1.4, 0.01}, DictT2Range -> {10., 70., 0.2}, DictT2fRange -> {100., 200., 2.}, 
...
EPGT2Fit[datan_, echoi_, angle_, OptionsPattern[]]:=Block[{
	...
	sig, dictMat, dictfMat, t2fran, S0c, met, val, out, T2fmap
	},
	...
	If[(OptionValue[EPGSmoothB1] && (OptionValue[EPGMethod]=!="NLLS") && !VectorQ[datal]),
		...
		(*smooth the B1 map*)
		B1Map = Clip[N[Round[LapFilt[B1Map] - b1ran[[1]], b1ran[[3]]] + b1ran[[1]]], b1ran[[1;;2]], {0., 0.}];
		...
		sol =If[val==2,
			...
			,
			(*smooth the T2fmat*)
			T2fmap = Clip[N@Round[LapFilt[T2fmap] - t2fran[[1]], t2fran[[3]]] + t2fran[[1]], t2fran[[1 ;; 2]], {0., 0.}];
...
DictionaryMinSearchi[{dict_, vals_}, ydat_, {maxx_,maxy_}] := Block[{err, coor, ErrorFuncDic},
	...
	{err, coor} = Quiet@NMinimize[
		...
		Method -> {"NelderMead", "PostProcess" -> False, 
			"ExpandRatio" -> 5, "ShrinkRatio" -> 0.9, 
			"ReflectRatio" -> 1, "ContractRatio" -> 0.5}];
	(*find the min vals*)
