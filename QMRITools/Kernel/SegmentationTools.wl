...
loc can be \"HeadNeck\", \"Shoulder\", \"Torso\", \"Hip\", \"UpperLeg\", \"LowerLeg\", \"Arm\", or \"PosSide\" to copy the body 
position/side classification network."
...
"SurfaceDistance[ref, pred] gives the surface distance of segmentations ref and pred for class 1, assuming isotropic unit 
voxels. Use the vox argument for correct physical distances.
...
The Method option controls the distance metric: \"Mean\", \"Median\", \"RMS\", \"HD\", \"HD95\" (default), or \"Std\". A list of 
...
"TrainSegmentationNetwork[{inFol, outFol}] trains a segmentation network from scratch. Training data should be wxf files in inFol. 
inFol can also be a list of folders, in which case wxf files from all folders are used. Progress is saved each round to outFol.
TrainSegmentationNetwork[{inFol, outFol}, netCont] continues training. netCont can be \"Start\" to restart, a NetGraph, a wlnet file, 
or a previous outFol to continue from the last saved network.
Loss functions can be All or a subset of {\"Dice\", \"Jaccard\", \"Tversky\", \"MSD\", \"CE\", \"Focal\", \"TopK\"}.
If AugmentMask -> True self supervised pretraining is done instead: the network gets a single output channel, only the MSD and MAE 
losses are used, and the training data is masked and noise filled as generated by GetTrainData with AugmentMask -> True."
...
The input data can be out of memory in the form of a list of \"*wxf\" files that contain the data, segmentation and voxel size or a list of \"*.nii\" 
files in the form {{\"data.nii\", \"segmentation.nii\"}..}. The input data can be in memory in a list in the form {{data, segmentation, vox}..}
GetTrainData[data, batch size, patch, nClass] If nClass is set to an value n > 0 the segmentations are decoded in n classes.
If AugmentMask -> True the target becomes the clean data itself and the input is masked with noise filled blocks, for self supervised pretraining."
...
more than one region, the second indicates if it has holes."
...
and overlapping labels are resolved. Returns a merged integer segmentation."


AugmentTrainingData::usage = 
"AugmentTrainingData[{data, seg}, vox] augments data and segmentation consistently. 
AugmentTrainingData[{data, seg}, vox, aug] controls augmentation. aug can be True/False for all, or an association with any of the 
keys \"Flip\", \"Rotate\", \"Scale\", \"Noise\", \"Blur\", \"Bias\" (unspecified keys default to True).
AugmentTrainingData[data, vox] augments data only, returning the data without segmentation."

AugmentImageData::usage = 
"AugmentImageData[image, {rotate, flip}] augments the input image by rotating between -180 and 180 degrees and flipping. 
The inputs rotate and flip can be set to True or False.
...
"MakeChannelClassImage[data, label] makes a cross-sectional image of the channels data overlaid with a cross-sectional 
image of the classes label of a training dataset generated.
...
"MakeChannelClassGrid[data, label] makes a 3 x 3 grid of cross-sectional images of the channels data overlaid with a cross-sectional 
image of the classes label of a training dataset generated.
MakeChannelClassGrid[data, label, n] makes a n x n.
MakeChannelClassGrid[data, label, {n, m}] makes a n x m."

MakeChannelGrid::usage =
"MakeChannelGrid[data] makes a 3 x 3 grid of cross-sectional images of the channels data of a training dataset generated by GetTrainData.
MakeChannelGrid[data, n] makes a n x n.
MakeChannelGrid[data, {n, m}] makes a n x m."
...
"UseParallelKernels is an option for TrainSegmentationNetwork. If set to True (default), producer kernels are launched
as raw WSTP links, data loading and batch augmentation run on these separate kernels while NetTrain runs concurrently
on its own kernel, which can substantially speed up training when augmentation is the bottleneck. If set to \"Parallel\",
the older shared-queue Parallel-framework kernel pool is used instead. If set to {True, n} or {\"Parallel\", n} at most
n producer kernels are used, useful for debugging on a small scale. If set to False batches are generated in-process."
...
encoder blocks of a pretrained input network so their weights are not updated during training, useful for transfer learning 
from a pretrained encoder. Only applies when netCont is not \"Start\".
...
AugmentMask::usage =
"AugmentMask is an option for GetTrainData and TrainSegmentationNetwork. If set to True self supervised masked-pretraining data/mode is used instead
of segmentation data: the target becomes the clean data itself and the input is masked with noise filled blocks."
...
RestartLearningCycle::usage =
"RestartLearningCycle is an option for TrainSegmentationNetwork. If set to True and continuing training from a previous run, the one-cycle
learning rate schedule (including its warmup) is restarted from 0 and fit to the remaining rounds, instead of resuming mid-cycle where the
...
"CleanUpSegmentations is an option for PrepareTrainingData. If set to True the segmentations are cleaned up by removing holes 
reducing to one volume and smoothing."

TrainVoxelSize::usage =
"TrainVoxelSize is an option for PrepareTrainingData. It defines the voxel size of the training data. When set to 
Automatic the voxel size is that of the data."
...
Values can be Automatic, All, or a integer value. If All the distance map is calculated for the whole image. If 0 the 
distance map is only calculated inside the segmentation."
...
TrainSegmentationNetwork::nofiles = "No training data files found."

TrainSegmentationNetwork::nullbatch = "A producer link returned Null instead of a training batch."
...
ImportITKLabelsI[file_, outType_] := Block[{fileL, lines, muscleNames, muscleLabels},
...
FindBodyPos[class_, mon_, debug_] := Block[{selection, locations, locationsR, len, classI, classN, n, xVars, eVars, dVars,
	...
	(*legacy networks classify as "Lower"/"Upper" instead of "LowerLegs"/"UpperLegs"*)
	classIn = class /. {"Lower" -> "LowerLegs", "Upper" -> "UpperLegs"};
...
PatchesToData[patches_, location_, dim : {_?IntegerQ, _?IntegerQ, _?IntegerQ}, labs_?ListQ] := Block[{
	...
	If[labs === {},
		...
		(*if needed remove the overlap and merge*)
...
SegmentData[datI_, opts:OptionsPattern[]] := SegmentData[{datI, {1, 1, 1}}, "Body", opts] /; !MatchQ[datI, {_, _?VectorQ}]

SegmentData[datI_, what_, opts:OptionsPattern[]] := SegmentData[{datI, {1, 1, 1}}, what, opts] /; !MatchQ[datI, {_, _?VectorQ}]

SegmentData[{datI_, vox_?VectorQ}, opts:OptionsPattern[]] := SegmentData[{datI, vox}, "Body", opts]

SegmentData[{datI_, vox_?VectorQ}, what_, OptionsPattern[]] := Block[{
	...
	timeAll = First@AbsoluteTiming[
		{dev, max, mon, sDim, netOut} = OptionValue[{TargetDevice, MaxMemorySize, Monitor,
			SegmentationDimension, NetworkOutput}];
		...
		rescale = If[vox =!= {1, 1, 1}, {vox, If[sDim === "2D", {vox[[1]], 1.5, 1.5}, {6, 1.5, 1.5}]}, Automatic];
...
SyntaxInformation[SplitDataForSegmentation] = {"ArgumentsPattern" -> {_, _., _., OptionsPattern[]}};
...
ApplySegmentationNetwork[dat_, netI_, node_, OptionsPattern[]] := Block[{
	...
	If[net===$Failed, $Failed,
		...
		(*perform the segmentation*)
		If[node==="",
			(*reverses all the padding and cropping and merges the patches, labs={} averages instead of merging labeled components*)
			merge[p_, labs_:{}] := ReverseCrop[ArrayPad[
				PatchesToData[ArrayPad[#, -pad] & /@ p, Map[# + {pad, -pad} &, pts, {2}], dim, labs], -pad
			], dimO, crp];
		...
		(*give the output*)
		If[node==="", Switch[netOut, "Confidence", conf, "Both", {seg, conf}, _, seg], seg]
	]
]
...
FindPatchDim[net_, dim_, lim_] := Block[{
	...
	If[Length[inp]===2,
		...
		,
		(*3D network figure out net dimensions and allowed steps*)
		{dz, dy, dx} = inp;
...
TrainSegmentationNetwork[{inFol : (_?StringQ | {__?StringQ}), outFol_?StringQ}, netCont_, opts : OptionsPattern[]] := Block[{
		...
		nProducers, loadData, queueVars, produced, used, ready, activeProducers, trainingDone, index, deadL, nullCount,
		nVal, makeVal, maxProducers, links, patchNClass, batchOptsSeq, producerStatus, roundImage, freezeDepth, chanIn, lrMult,
		augMask, testDataRaw, dispDat, testFile, restartCycle, cycleRounds, cycleItt, valOut
	},
	...
	{batch, roundLength, rounds, augment, pad, patch, patches,
		loss, rep, learningRate, l2reg, multi, tar, freezeDepth, augMask, restartCycle} = OptionValue[
		{BatchSize, RoundLength, MaxTrainingRounds, AugmentData, PadData, PatchSize,
			...
			MultiChannel, TargetDevice, FreezeEncoderDepth, AugmentMask, RestartLearningCycle}];

	(*False, True, "Parallel", or {True|"Parallel",n} to cap producer count*)
	parallel = OptionValue[UseParallelKernels];
	{parallel, maxProducers} = Which[
		parallel === True || parallel === "Parallel", {parallel, Automatic},
		MatchQ[parallel, {True | "Parallel", _Integer?Positive}], {First[parallel], Last[parallel]},
	...
	If[files === {}, 
		Return[Message[TrainSegmentationNetwork::nofiles]; $Failed],
		MonitorFunction[Length@files, "Files for training: "];
	];

	(*------------ get the test data -----------------*)

	testDataRaw = Normal/@Import[First@files];
	...
	{netIn, ittTrain} = Which[
		...
		Which[
			...
			If[FileExistsQ[netCont], {Import[netCont], 0}, Return[$Failed]],
			...
			If[Length[netIn] > 0, 
				...
				Return[Message[TrainSegmentationNetwork::net];
					Message[TrainSegmentationNetwork::cont]; $Failed]
			],
			True, Return[Message[TrainSegmentationNetwork::net]; 
				Message[TrainSegmentationNetwork::inp]; $Failed]
		],
	...
	(*define the network for training, only add the loss layers that are actually used*)
	netIn = AddLossLayer[netIn, loss];
	(*AddLossLayer nests the network under the "net" key, the frozen positions must follow*)
	lrMult = ({"net", #[[1]]} -> #[[2]]) & /@ lrMult;
	...
	With[{mon = <|"outName" -> outName, "ittString" -> ittString, "testData" -> testData,
		...
		monitorFunction = (
			...
			(*net auto-batches the list of 2D slice examples*)
			testSeg = netMon[mon["testData"], TargetDevice -> "CPU"];
			If[!mon["augMask"], testSeg = Ramp[ClassDecoder[testSeg]]];
			ExportNii[testSeg, mon["testVox"], base[ittTrain,".nii"]];
			(*transpose 2D to the {channel, slice, ...} shape the grid functions expect*)
			dispDat = If[mon["is2D"], Transpose@mon["testData"], mon["testData"]];
			(*pretraining shows corrupted vs reconstructed, otherwise the class overlay grid*)
			im = If[mon["augMask"], ImageAssemble[{{MakeChannelGrid[dispDat, 3], MakeChannelGrid[{testSeg}, 3]}}],
				MakeChannelClassGrid[dispDat, {testSeg, {0, mon["nClass"]-1}}, 3]];
	...
	(*batch function*)
	patchNClass = {patch, nClass};
	batchOptsSeq = Sequence[PatchesPerSet -> patches, AugmentData -> augment, PadData -> pad, AugmentMask -> augMask];
	With[{patch = patchNClass, batchOpts = batchOptsSeq},
	...
	(*RestartLearningCycle fits a fresh cycle to the remaining rounds instead of resuming mid-cycle*)
	{cycleRounds, cycleItt} = If[restartCycle, {rounds - ittTrain, 0}, {rounds, ittTrain}];
	schedule = OneCycleSchedule[roundLength / batch, cycleRounds, cycleItt];
	...
	(*spin up producer kernels if needed*)
	If[parallel =!= False,
		{links, nProducers} = LaunchTrainingKernels[parallel, maxProducers];
		MonitorFunction[nProducers, "Producer kernels launched: "];
	];

	(*import all train data or train out of memory and create 20% of round as validation*)
	{data, valOut, dims} = LoadTrainData[parallel, files,
		{loadData, makeVal, nVal, batchFunction}, {links, nProducers, patchNClass, {batchOptsSeq}}];

	(*keep the disk-loaded validation set when a fresh one wasn't needed*)
	If[makeVal, validation = valOut; 
	...
	(*shared producer/consumer status monitor - showReady hides the Ready row for the Link path, which has no ready array*)
	producerStatus[showReady_:True] := Grid[Join[{
		...
		If[showReady, {Join[{Style["Ready:", Bold], Count[ready, True], " - "}, Boole[ready]]}, {}]
	], Alignment -> Center];
	...
	Switch[parallel,
		...
		True,
			...
			used = Table[0, nProducers];
			deadL = Table[False, nProducers];
			nullCount = Table[0, nProducers];
			index = 0;
			(*activate every producer once - batchFunctionL already has patch/batchOpts baked in*)
			With[{b = batch}, LinkWrite[#, Unevaluated[batchFunctionL[b]]]]& /@ links;
			produced = Table[1, nProducers];

			(*Monitor training and producing*)
			PrintTemporary[Dynamic[producerStatus[False]]];
			PrintTemporary[Dynamic[roundImage[]]];

			(*train using the links as the batch source, always close the links after - trained or aborted*)
			CheckAbort[
				trained = trainFunc[GetFromBatchQueueL[links, nProducers, batch]&],
				LinkClose /@ links; Abort[]
			];

			(*Final producer feedback*)
			MonitorFunction[producerStatus[False]];
			LinkClose /@ links;
		,
		"Parallel",
...
MakeTestData[data_, n_, patch_, corrupt_?BooleanQ] := Block[{testData, len, sel, is2D, stack, mask},
	...
	If[is2D,
		...
		If[len > First@patch && Length@patch===3,
			sel = Range @@ Clip[Round[(Clip[Round[len/3 - (0.5 n) First@patch],
				{0, Infinity}] + {1, n First@patch})], {1, len}, {1, len}];
			testData = First@AutoCropData[testData[[sel]]]
		]
	];

	(*pad and normalize same as batch selection*)
	testData = If[is2D,
...
FreezeEncoderLayers[net_, n_Integer, includeStart_?BooleanQ] := Block[{nodes},
	...
	(*keep only the first n encoder blocks*)
...
LaunchTrainingKernels[parallel_, maxProducers_:Automatic] := Switch[parallel,
	True,
	Block[{load, mx, context, nProducers, links, status, failed, dead},
		(*start the links*)
		nProducers = If[maxProducers === Automatic, $ProcessorCount, maxProducers];
		links = Table[LinkLaunch[First[$CommandLine] <> " -wstp"], nProducers];

		(*load the package/bootstrap per link, then drain that response before sending anything else*)
		load = If[StringContainsQ[First[PacletFind["QMRITools"]]["Location"], "workspace"],
			"QMRIToolsDev`", "QMRITools`"];
		mx = QMRITools`SegmentationTools`Private`SetMXenvironment;
		MonitorFunction[nProducers, "Starting link kernels: "];
		With[{l = load, m = mx}, LinkWrite[#, Unevaluated[
			Get[l];
			m["StartTrain"];
		...
		(*confirm QMRITools is actually available*)
		context = "QMRITools`SegmentationTools`";
		LinkWrite[#, Unevaluated[Context[GetTrainData]]]& /@ links;
		status = AssociationThread[links -> (ReadLinkL[#] === context & /@ links)];

		(*retry by waiting longer, not asking again - the original request may still be in flight*)
		failed = Keys[Select[status, ! # &]];
		If[failed =!= {},
			AssociateTo[status, Thread[failed -> (ReadLinkL[#, 120] === context & /@ failed)]];
			dead = Keys[Select[status, ! # &]];
			LinkClose /@ dead;
			status = KeyDrop[status, dead];
		];

		MonitorFunction[{Length@status, Values[status]}, "QMRITools context per link kernel: "];
		{Keys[status], Length[status]}
	],
	"Parallel",
	With[{
		setMX = SetMXenvironment,
			...
			(*restart the kernel pool clean, then load the package on every kernel*)
			CloseKernels[];
...
ReadLinkL[link_, timeout_:60] := Block[{pkt, t0, $IterationLimit},
	$IterationLimit = Infinity; (*prevent recusrsion limit abort*)
	t0 = AbsoluteTime[];
	(*reads the next ReturnPacket from a link, discarding other packets in between*)
	While[True,
		If[AbsoluteTime[] - t0 > timeout, Return[$Failed]];
		If[LinkReadyQ[link],
			pkt = Check[LinkRead[link], $Failed];
			Switch[pkt,
				$Failed, Return[$Failed],
				_ReturnPacket, Return[First[pkt]],
				_, Null
			],
			Pause[0.02]
		]
...
LoadTrainData[parallel_, files_, {loadData_, makeVal_, nVal_, batchFunction_}, {links_, nProducers_, patchNClass_, batchOptsSeq_}] := Switch[parallel,
	...
	(*parallel training with link*)
	True, Block[{pFiles, dims, validation},
		pFiles = PartitionProducerFiles[files, nProducers];
		(*each link imports its own file share*)
		With[{ld = loadData}, MapThread[LinkWrite[#1, Unevaluated[
			data = If[ld, Import /@ #2, #2];
		]]&, {links, pFiles}]];
		ReadLinkL[#, 120]& /@ links;
		(*define the reusable batchFunctionL[n] per link, patch/batchOpts baked in once*)
		With[{p = patchNClass, bo = batchOptsSeq}, LinkWrite[#, Unevaluated[
			batchFunctionL[n_] := GetTrainData[data, n, p, Sequence @@ bo]
		]]& /@ links];
		ReadLinkL /@ links;

		(*dims step*)
		With[{ld = loadData}, LinkWrite[#, Unevaluated[
			If[ld, If[ArrayDepth[#] === 3, Dimensions[Transpose[{#}]], Dimensions[#]] & /@ data[[All, 1]], {}]
		]]& /@ links];
		dims = ReadLinkL /@ links;

		(*validation step - reuses batchFunctionL*)
		With[{mv = makeVal, nv = Round[nVal / nProducers]}, LinkWrite[#, Unevaluated[
			If[mv, batchFunctionL[nv], {}]
		]]& /@ links];
		validation = ReadLinkL /@ links;

		(*data stays on the links - only dims/validation come back*)
		{{}, Join @@ validation, Join @@ dims}
	],

	(*parallel training with parallel kernels*)
	"Parallel", Block[{pFiles, jobs, dims, validation},
		pFiles = PartitionProducerFiles[files, nProducers];
		DistributeDefinitions[pFiles, nProducers, nVal, batchFunction];
		(*one submit per worker that loads its own file share, plus one empty submit for the fitter kernel*)
		jobs = ParallelSubmit[
			data = If[loadData, Import /@ #, #];
			dims = If[loadData, If[ArrayDepth[#] === 3,
				Dimensions[Transpose[{#}]], Dimensions[#]] & /@ data[[All, 1]], {}];
			validation = If[makeVal, batchFunction[Round[nVal / nProducers]], {}];
			{dims, validation}
		]& /@ pFiles;
		(*wait for all workers, drop the empty fitter-kernel result*)
		{dims, validation} = Transpose[Most[WaitAll[Append[jobs, ParallelSubmit[{}]], ProgressReporting -> False]]];
		(*data stays on the workers - only dims/validation come back*)
		{{}, Join @@ validation, Join @@ dims}
	]
]
...
GetFromBatchQueueL[links_, nP_, batch_] := Block[{startTime, link, result, reactivate},
	(*requests the next batch from the link just read*)
	reactivate[] := With[{b = batch}, LinkWrite[link, Unevaluated[batchFunctionL[b]]]];
	startTime = AbsoluteTime[];
	(*round-robin poll the links until one throws a result*)
	Catch[While[True, Do[
		index = Mod[index, nP] + 1;
		If[deadL[[index]], Continue[]];
		link = links[[index]];
		If[LinkReadyQ[link],
			result = ReadLinkL[link];
			Which[
				(*Null means GetTrainData failed - close after repeated failures*)
				result === Null,
					nullCount[[index]]++;
					If[nullCount[[index]] >= 10,
						Message[TrainSegmentationNetwork::nullbatch];
						LinkClose[link]; deadL[[index]] = True; activeProducers--,
						reactivate[]
					],
				result === $Failed,
					deadL[[index]] = True; activeProducers--,
				True,
					used[[index]]++;
					reactivate[];
					produced[[index]]++;
		...
		(*no producers left, or none delivered in time, give up*)
...
GetKernels[nProducers_, batch_, batchFunction_, trainFunc_] := Append[
	(*one producer kernel per queue slot, filling it whenever the trainer has consumed it*)
	Block[{qi = #, chunk}, ParallelSubmit[
...
(*If[produced[[qi]]===1, Print["run: ",Dimensions/@ #& /@ ToExpression["queue" <> ToString[qi]]]];*)
	...
	,
	(*the trainer kernel, sourcing batches by round-robin polling the queue slots*)
	ParallelSubmit[CheckAbort[First@{trainFunc[
		(*the batch function*)
		Block[{startTime, chunk},
			startTime = AbsoluteTime[];
			Catch[While[True, Do[
...
(*named augmentation switches, in arbitrary order, default is all True*)
AugmentationDefaults = <|"Flip" -> True, "Rotate" -> True, "Scale" -> True, "Noise" -> True, "Blur" -> True, "Bias" -> True|>;
...
AugmentTrainingData[{dat_?ArrayQ, seg_?ArrayQ}, vox_, opts:OptionsPattern[]] :=
	AugmentTrainingDataI[{dat, seg}, vox, AugmentationDefaults, opts]

AugmentTrainingData[{dat_?ArrayQ, seg_?ArrayQ}, vox_, aug_?BooleanQ, opts:OptionsPattern[]] :=
	AugmentTrainingDataI[{dat, seg}, vox, AssociationMap[aug&, Keys@AugmentationDefaults], opts]

AugmentTrainingData[{dat_?ArrayQ, seg_?ArrayQ}, vox_, aug_?AssociationQ, opts:OptionsPattern[]] :=
	AugmentTrainingDataI[{dat, seg}, vox, Join[AugmentationDefaults, aug], opts]

AugmentTrainingData[dat_?ArrayQ, vox_, opts:OptionsPattern[]] :=
	AugmentTrainingDataI[{dat, 1}, vox, AugmentationDefaults, opts]

AugmentTrainingData[dat_?ArrayQ, vox_, aug_?BooleanQ, opts:OptionsPattern[]] :=
	AugmentTrainingDataI[{dat, 1}, vox, AssociationMap[aug&, Keys@AugmentationDefaults], opts]

AugmentTrainingData[dat_?ArrayQ, vox_, aug_?AssociationQ, opts:OptionsPattern[]] :=
	AugmentTrainingDataI[{dat, 1}, vox, Join[AugmentationDefaults, aug], opts]


AugmentTrainingDataI[{dat_?ArrayQ, seg_}, vox_, aug_?AssociationQ, OptionsPattern[]] := Block[{
	datT, segT, cr, w, r, t, s, dim, flip, rot, trans, scale, noise, blur, bias, isNot2D},

	(*prep data*)
	{flip, rot, scale, noise, blur, bias} = Lookup[aug, {"Flip", "Rotate", "Scale", "Noise", "Blur", "Bias"}];
	isNot2D = !OptionValue["Augment2D"];
	datOnly = OptionValue["AugmentMask"] || seg===1;

	datT = ToPackedArray[N[dat]];
	If[!datOnly, segT = ToPackedArray[N[seg]]];

	(*Augmentation of mirroring*)
	If[flip && Coin[], If[datOnly, 
		datT = ReverseC[datT],
		{datT, segT} = ReverseC[{datT, segT}]
	]];
	...
	If[datOnly, datC = datT];
	(*Augmentations sharpness, either blur or unsharp mask sharpen, only sharpening real signal not background noise*)
	If[blur && Coin[],
		s = GaussianFilter[datT, RandomReal[{0.5, 2}]];
		datT = If[Coin[], s, datT + Unitize[datT] RandomReal[{0.5, 1.5}] Ramp[(datT - s)]]
	];
	...
	If[OptionValue["AugmentMask"],
		{ToPackedArray[N[datT]], ToPackedArray[N[datC]]},
		If[seg===1,
			ToPackedArray[N[datT]],
...
MakeBlockMask[dim_, n_:1] := Table[SparseArray[DeleteDuplicates[makeBoxC[dim]] -> 1, dim],{i,n}];

makeBoxC = Compile[{{dim, _Integer, 1}}, Block[{
		out, len, rand, size, mc, center, raw, coor, ax, ay, az, theta, 
		mat, rot, box, noise, noiseN, diff
	},
	
	len = Times @@ dim;
	rand = 2 RandomReal[{0.1, .5}] len;
	noiseN = Round[RandomReal[{0, 0.005}] len];
	out = Transpose[Ceiling[dim RandomReal[{0, 1}, {3, noiseN}]]];
	
	While[Length[out] < rand,
		noise = Transpose[Ceiling[dim RandomReal[{0, 1}, {3, noiseN}]]];
		
		raw = RandomReal[{-1, 1}, 3];
		{ax, ay, az} = raw/Sqrt[raw . raw];
		theta = RandomReal[{0, 2 Pi}];
		mat = {{0, -az, ay}, {az, 0, -ax}, {-ay, ax, 0}};
		rot = {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}} + Sin[theta] mat + (1 - Cos[theta]) mat . mat;
		
		size = Round[1.25 RandomReal[#] & /@ Transpose[{dim/8, dim/2}]];
...
GetTrainData[dataSets_, nBatch_, {patch_, nClass_}, OptionsPattern[]] := Block[{
		itt, datO, segO, dat, seg, vox, augI, aug, nSet, pad, sel, is2D, augMask, nP, mask
	},
	...
	is2D = Length[patch]===2;
	{augI, nSet, pad, augMask} = OptionValue[{AugmentData, PatchesPerSet, PadData, AugmentMask}];
	{aug, is2D} = Which[
	...
	While[Length@datO < nBatch,
		...
		(*for self supervised pretraining the segmentation is not needed, the clean data is used as target*)
		{dat, seg} = If[augMask,
			AugmentTrainingData[dat, vox, aug, "Augment2D" -> is2D, "AugmentMask" -> True],
			AugmentTrainingData[{dat, Normal@seg}, vox, aug, "Augment2D" -> is2D]
		];
...
PatchTrainingData[{dat_, seg_}, patch_, n_] := Block[{
	...
	{datP, segP} = Switch[pLen,
		...
		{ToPackedArray[N[datP[[1, slice]]]] , ToPackedArray@segP[[1, slice]]},
		3,
		{ToPackedArray[N[datP]], ToPackedArray@segP}
	];
...
(*shared by MakeChannelClassGrid and MakeChannelGrid, picks an n1 x n2 grid and the len indices to fill it*)
GridLayout[len_, ni_] := Block[{n1, n2},
	If[IntegerQ[ni],
...
MakeChannelClassGrid[dat_, lab_, ni_] := Block[{datN, len, n1, n2, ind, labP, ran},
	datN = Normal@dat;
	len = Length@First@Normal[datN];
	{n1, n2, ind} = GridLayout[len, ni];

	labP = Normal@lab;
	{labP, ran} = If[TensorQ[labP], {labP, MinMax@labP}, labP];
	If[IntegerQ[Round[ran]], ran = {0, ran}];

	RemoveAlphaChannel@ImageAssemble@Partition[
		ImagePad[MakeChannelClassImage[datN[[{1}, #]], labP[[#]], ran], 4, Black
	] & /@ ind, n1]
]
...
SyntaxInformation[MakeChannelGrid] = {"ArgumentsPattern"->{_, _.}};

MakeChannelGrid[dat_] := MakeChannelGrid[dat, 3]

MakeChannelGrid[dat_, ni_] := Block[{datN, len, n1, n2, ind},
	datN = Normal@dat;
	len = Length@First@Normal[datN];
	{n1, n2, ind} = GridLayout[len, ni];

	RemoveAlphaChannel@ImageAssemble@Partition[
		ImagePad[ImageCollage@MakeChannelImage[datN[[{1}, #]]], 4, Black
	] & /@ ind, n1]
]
...
MakeClassImage[labelI_,{offI_?NumberQ, maxI_?NumberQ}, vox_?VectorQ] := Block[{max, cols, imLab, rat, label, off},
	...
	*)
	{label, off, max} = Round[{Normal@labelI, offI, maxI}];
	max = Max[{Max[label], max}];
	(*explicit {off,max} range, not the list's own min/max, so a single foreground class doesn't divide by zero*)
	cols = Prepend[ColorData["RomaO"][#]&/@Rescale[
			Join[Select[Range[off + 1, max], EvenQ], Select[Range[off + 1, max], OddQ]], {off, max}
		],Transparent];
...
SyntaxInformation[MakeChannelImage]={"ArgumentsPattern"->{_, _.}};

MakeChannelImage[data_] := MakeChannelImage[data, {1, 1, 1}]

MakeChannelImage[data_, vox_] := Block[{dat, imDat, rat, ran},
	dat = Normal@data;
...
ShowTrainLog[fol_, max_] := DynamicModule[{
	...
	Manipulate[
		...
		Delimiter,
		(*key2 is empty when the net has a single loss port, NetTrain then logs no per-port named loss to toggle,
		hide the control rather than give TogglerBar an empty option list*)
		Control[{{key, {}, ""}, If[key2 === {}, {"None"}, key2],
		ControlType -> If[key2 === {}, None, TogglerBar],
		Appearance -> "Vertical" -> {Automatic, 4}, BaseStyle -> Medium}],
		Delimiter,
