...
If AugmentMask -> True self supervised pretraining is done instead: the network gets a single output channel, only the MSD and MAE losses are used, and the training data is masked and noise filled as generated by GetTrainData with AugmentMask -> True."
...
AugmentTrainingData[{data, seg}, vox, aug] controls augmentation. aug can be True/False for all, or an association with any of the keys \"Flip\", \"Rotate\", \"Scale\", \"Noise\", \"Blur\", \"Bias\" (unspecified keys default to True).
...
TrainSegmentationNetwork[{inFol : (_?StringQ | {__?StringQ}), outFol_?StringQ}, netCont_, opts : OptionsPattern[]] := Block[{
	...
	{batch, roundLength, rounds, augment, pad, patch, patches,
		loss, rep, learningRate, l2reg, multi, tar, freezeDepth, augMask} = OptionValue[
		{BatchSize, RoundLength, MaxTrainingRounds, AugmentData, PadData, PatchSize,
			...
			MultiChannel, TargetDevice, FreezeEncoderDepth, AugmentMask}];
	...
	With[{
			patch = {patch, nClass},
			batchOpts = Sequence[PatchesPerSet -> patches, AugmentData -> augment, PadData -> pad, AugmentMask -> augMask]
		},
