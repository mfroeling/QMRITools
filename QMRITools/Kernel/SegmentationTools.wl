...
If MaskedPretraining -> True self supervised pretraining is done instead: the network gets a single output channel, only the MSD and MAE
losses are used, and the training data is masked and noise filled as generated by GetTrainData with MaskedPretraining -> True."
...
If MaskedPretraining -> True the target becomes the clean data itself and the input is masked with noise filled blocks, for self supervised pretraining."
...
MaskedPretraining::usage =
"MaskedPretraining is an option for GetTrainData and TrainSegmentationNetwork. If set to True self supervised masked-pretraining data/mode is used instead
...
SegmentData[datI_, opts:OptionsPattern[]] := SegmentData[{datI, {1, 1, 1}}, "Body", opts] 
...
SegmentData[{datI_, vox_?VectorQ}, what_?StringQ, OptionsPattern[]] := Block[{
	...
	timeAll = First@AbsoluteTiming[
		...
		rescale = If[vox =!= {1, 1, 1}, N@{vox, If[sDim === "2D", {vox[[1]], 1.5, 1.5}, {6., 1.5, 1.5}]}, Automatic];
...
TrainSegmentationNetwork[{inFol : (_?StringQ | {__?StringQ}), outFol_?StringQ}, netCont_, opts : OptionsPattern[]] := Block[{
	...
	makeTest[] := (
		{testData, testVox} = MakeTestData[testDataRaw, 2, patch, pretrain];
		ExportNii[If[is2D, testData[[All, 1]], First@testData], testVox, testFile];
	);

	(*reuse the test set if present and matching, else make and export a new one*)
	...
	With[{mon = <|"outName" -> outName, "ittString" -> ittString, "testData" -> testData,
			"testVox" -> testVox, "nClass" -> nClass, "is2D" -> is2D, "pretrain" -> pretrain|>},
		monitorFunction = (
			ittTrain += rep;
			...
			If[!mon["pretrain"], testSeg = Ramp[ClassDecoder[testSeg]]];
			...
			im = If[mon["pretrain"], ImageAssemble[{{MakeChannelGrid[dispDat, 3], MakeChannelGrid[{testSeg}, 3]}}],
	...
	batchOptsSeq = Sequence[PatchesPerSet -> patches, AugmentData -> augment, PadData -> pad, MaskedPretraining -> pretrain];
	...
	trainOpts = Sequence[
		...
		LearningRate -> learningRate, Method -> {"ADAM", "L2Regularization" -> l2reg,
			"LearningRateSchedule" -> schedule, "Beta1" -> 0.9, "Beta2" -> 0.998,
			"Epsilon" -> 10^-4, "GradientClipping" -> 1},
	...
	Switch[parallel,
		...
		True,
			...
			used = produced = nullCount = Table[0, nProducers];
			deadL = Table[False, nProducers];
			index = 0;

			(*activate every producer once - batchFunctionL already has patch/batchOpts baked in*)
			Table[With[{b = batch}, LinkWrite[links[[i]], Unevaluated[batchFunctionL[b]]]];
				produced[[i]]++; Pause[.1]; , {i, nProducers}];
...
LaunchTrainingKernels[parallel_, maxProducers_:Automatic] := Switch[parallel,
	...
	Block[{load, mx, context, nProducers, links, status, failed, dead},
		...
		With[{l = load, m = mx}, LinkWrite[#, Unevaluated[
			Get[l];
			m["StartTrain"];
			SetSystemOptions["ParallelOptions" -> {"MKLThreadNumber" -> 4, "ParallelThreadNumber" -> 1}]
		]]& /@ links];
		ReadLinkL[#, 120]& /@ links;
	...
	With[{
		setMX = SetMXenvironment,
			...
			Quiet@ParallelEvaluate[
				...
				Quiet@System`SetSystemOptions["ParallelOptions" -> {"MKLThreadNumber" -> 4, "ParallelThreadNumber" -> 1}]
			, ProgressReporting -> False];
			{{}, nKernels - 1}
		]
	]
...
GetKernels[nProducers_, batch_, batchFunction_, trainFunc_] := Append[
	(*qi is spliced via With, not Block, since the remote kernel doesn't share master's dynamic scope*)
	With[{qi = #}, Block[{chunk}, ParallelSubmit[
...
AugmentTrainingDataI[{dat_?ArrayQ, seg_}, vox_, aug_?AssociationQ, OptionsPattern[]] := Block[{
	...
	datOnly = OptionValue["MaskedPretraining"] || seg === 1;
	...
	If[OptionValue["MaskedPretraining"],
...
GetTrainData[dataSets_, nBatch_, {patch:{__Integer}, nClass_}, OptionsPattern[]] := Block[{
	...
	While[Length@datO < nBatch,
		...
		{dat, seg} = If[pretrain,
			AugmentTrainingData[dat, vox, aug, "Augment2D" -> is2D, "MaskedPretraining" -> True],
