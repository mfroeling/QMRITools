...
"FitGradientMap[data] fits a polynomial gradient field of order 2 to data, sampling every voxel.
FitGradientMap[data, ord] fits a polynomial gradient field of order ord to data, sampling every voxel.
FitGradientMap[data, ord, smp] fits a polynomial gradient field of order ord to data, sampling every smp voxels.
FitGradientMap[{data, mask}, ord, smp] same but only fits within mask."
...
CutData::usage = 
"CutData[data] splits the data in two sets left and right base on a automated algorithm assuming leg data.
CutData[data, cut] splits the data in two sets left and right at location cut.
CutData[data, {cut, over}] splits the data in two sets left and right at location cut but with overlap defined by over."

StichData::usage =
"StichData[dataLeft, dataRight] joins left and right part of the data generated by CutData."

FindMiddle::usage=
"FindMiddle[data] find the center of a two leg data, assuming leg data.
FindMiddle[data, \"Legs\"] same as FindMiddle[data], the only anatomy currently supported."
...
LapFilter::usage =
"LapFilter[data] Laplacian filter of data with kernel size 0.5.
...
"GyromagneticRatio[nuclei] gives the gyromagnetic ratio for the nuclei, e.g. \"31P\" or \"1H\", in MHz/T."
...
"DecomposeAffineMatrix[mat] decomposes the affine matrix in T, R, S and Q."

DecomposeScaleMatrix::usage =
"DecomposeScaleMatrix[S] decomposes the scale matrix in S1, S2 and S3."
...
LightDarkV[light, dark] does the same as LightDarkSwitched but only for version 14.3 or higher."
...
CropOutput::usage =
"CropOutput is an option for CropData, can be \"All\" (data and crop values), \"Data\" (just the cropped data) or \"Crop\" (just the crop values)."
...
SaveImage[exp_, fileIn_String, OptionsPattern[]] := Module[{file,imSize,res,type},
	type = OptionValue[FileType];
	type = If[StringTake[type, 2] === "*.", StringDrop[type,1], If[StringTake[type, 1] === ".",type, "."<>type]];

	file=If[StringTake[fileIn,-4]===type||StringTake[fileIn,-5]===type,fileIn,fileIn<>type];
...
RescaleDataI[data_?ArrayQ, sc_?VectorQ, met_, opts : OptionsPattern[]] := Block[{type, dim, int, dataOut},
	...
	dataOut = Switch[ArrayDepth[data],
		...
		Switch[Length[sc],
			2 (*rescale a stack of 2D images*),
...
GridData[dataIn_, opts:OptionsPattern[]] := GridData[dataIn, Ceiling[Sqrt[Length@dataIn]], opts]

GridData[dataIn_, part_, opts:OptionsPattern[]] := Block[{dim, data, aDepth, pad, val},
	aDepth = ArrayDepth[dataIn[[1]]];
	...
	data = If[pad =!= None,
		...
		ArrayPad[#, pad, val] & /@ dataIn
		,
		ToPackedArray@N@dataIn
	];
...
DataToVector[dataIn_] := DataToVector[dataIn, 1]

DataToVector[dataIn_, maskIn_] := Module[{data, sp, mask, depthDat, depthMask, depth, dimMask, dimDat, vec, coor, min},
	data = ToPackedArray[N[dataIn]];
	dimDat = Dimensions[data];
	depthDat = ArrayDepth[data];

	(*data dimensions are not correct, mask must be 2D or 3D*)
	If[! (depthDat == 2 || depthDat == 3 || depthDat == 4), 
	Return@Message[DataToVector::dim, "Data", depthDat]];
	If[maskIn =!= 1,
		(*masks is given*)
		mask = ToPackedArray@Round@maskIn;
		dimMask = Dimensions[mask];
		depthMask = ArrayDepth[mask];
		
		(*check mask*)
		depth = depthDat - depthMask;
		If[! (depthMask == 2 || depthMask == 3), Message[DataToVector::dim, "Mask", depthMask]];
		dimDat = If[depth == 0,
			(*mask and data are same dimensions*)
			dimDat,
			(*data is one dimensions larger than mask either 2D and 3D or 3D and 4D*)
			dimDat = If[depth == 1 && depthDat == 3, Drop[dimDat, 1], Drop[dimDat, {2}]]
		];
		(*Dimensions must be equal*)
		If[dimDat =!= dimMask, Return@Message[DataToVector::mask, dimDat, dimMask]];
		,
		(*mask is not given make mask*)
		mask = If[depthDat == 4, Unitize[Total@Transpose@data], 1]
	];

	If[mask === 1,
		sp = SparseArray[data, Dimensions@data, 0.];
		{sp["ExplicitValues"], {dimDat, sp["ExplicitPositions"]}}
		,

		mask = SparseArray[Round@mask, Dimensions@mask, 0];
		(*Flatten the data*)
		data = If[depthDat == 4, 
			Transpose[data, {1, 4, 2, 3}], 
			If[depthDat == 3 && depth == 1, RotateDimensionsLeft[data], data]
		];

		(*get the data and positions there mask is 1*)
		coor = mask["ExplicitPositions"];
		vec = If[Head[data]===SparseArray,
			(*to prevent memory blowup for large sparse arrays*)
			min = (-Min[data] + 1) mask;
			vec = mask data + min;
			vec = vec["ExplicitValues"];
...
Options[CutData] = {Monitor -> False};


SyntaxInformation[CutData] = {"ArgumentsPattern" -> {_,_.}}

CutData[data_, opts:OptionsPattern[]] := CutData[data, FindMiddle[data, opts]]

CutData[data_, cut_?IntegerQ, opts:OptionsPattern[]] := CutData[data, {cut, 0}, opts]

CutData[data_, {cut_?IntegerQ, over_?IntegerQ}, opts:OptionsPattern[]] := Switch[ArrayDepth[data],
	4, {data[[All, All, All, ;; cut + over]], data[[All, All, All, (cut - over + 1) ;;]], cut},
	3, {data[[All, All, ;; cut + over]], data[[All, All, (cut - over + 1) ;;]], cut}
]
...
Options[FindMiddle] = {Monitor -> False};


SyntaxInformation[FindMiddle] = {"ArgumentsPattern" -> {_, _.}}

FindMiddle[dataIn_, opts:OptionsPattern[]] := FindMiddle[dataIn, "Legs", opts]

FindMiddle[dataIn_, "Legs", opts:OptionsPattern[]] := Block[{
		dat, fDat, len, datf, peaks, mid, peak, center, mask, ran, blur, i, max
	},
	...
	If[peaks==={},
		...
		If[OptionValue[Monitor], MonitorFunction[Show[
...
StichData[dataLeft_, dataRight_] := RotateDimensionsLeft[Join[RotateDimensionsRight[dataLeft], RotateDimensionsRight[dataRight]]];
...
QMRIToolsFunctions[toolbox_?StringQ] := QMRIToolsFunctions[{toolbox}, 0]

QMRIToolsFunctions[toolbox : {_?StringQ ..}] := QMRIToolsFunctions[toolbox, 0]

QMRIToolsFunctions[p_?IntegerQ] := QMRIToolsFunctions[QMRIToolsPackages[], p]

QMRIToolsFunctions[toolbox_?StringQ, p_?IntegerQ] := QMRIToolsFunctions[{toolbox}, p]

QMRIToolsFunctions[toolbox : {_?StringQ ..} | {"All"}, p_?IntegerQ] := Block[{
		pack, ind, contexts, allNames, func, opts, names, out
	},

	{pack, ind} = If[toolbox === {"All"}, {QMRIToolsPackages[], False}, {toolbox, True}];
...
QMRIToolsFuncPrint[toolbox_String] := If[toolbox=="",PrintAll/@QMRIToolsFunctions[];,PrintAll[QMRIToolsFunctions[toolbox]];]
...
SumOfSquares[dataIn_, OptionsPattern[]] := Block[{sos, weights, data},
	data = RotateDimensionsLeft[dataIn];
	sos = SumOfSquaresI[data];

	If[OptionValue[OutputWeights],
		weights = DivideNoZero[data, sos];
...
NNLeastSquares[a_, y_] := Block[{at, x, zeroed, w, zeroW, pos, sp, xp, neg, xi, si, alpha, i, j ,l},
	...
	While[j < l && Total[zeroed] > 0. && Max[zeroW = zeroed w] > 0,
		...
		(*recalculate the solutions of sp until all values of sp are positive*)
		i = 1;
		While[i < l && Min[sp] < 0.,
			i++;
			...
			(*removed the lowest value of sp from the positive set*)
...
PosInd = Compile[{{v, _Real, 1}}, Block[{z = Round@Total[1 - v]}, 
	Ordering[v][[;; z]]], RuntimeOptions -> {"Speed", "WarningMessages" -> False}];

CalcW = Compile[{{A, _Real, 2}, {At, _Real, 2}, {y, _Real, 1}, {x, _Real, 1}}, 
	Chop[At . (y - A . x)], RuntimeOptions -> {"Speed", "WarningMessages" -> False}];

LLSC = Compile[{{A, _Real, 2}, {y, _Real, 1}}, 
	Inverse[A . Transpose[A]] . A . y, RuntimeOptions -> {"Speed", "WarningMessages" -> False}];
...
BSplineCurveFit[pts_, opts : OptionsPattern[]] := Block[{paras, knots, coefficientMat, controlPoints, cpn, sd, reg, len, Amat, ptsP},
	len = Length[pts];
	cpn = Min[{len - 2, OptionValue[SplineKnotsNumber]}];

	{coefficientMat, Amat} = BSplineBasisFunctions[len, opts];
	ptsP = PadRight[pts, Length[Amat]];
	controlPoints = LLeastSquares[Amat, ptsP];
	(Amat . controlPoints)[[;; (-cpn - 1)]]
	]
...
BSplineBasisFunctions[nPts_, opts : OptionsPattern[]] := BSplineBasisFunctions[nPts, opts] = Block[{
	cpn, sd, reg, len, paras, knots, coefficientMat, coefficientMatR, coefficientMatDD, smooth
	},
	(*Get the options*)
	...
	(*define the b-spline points x = [0,1]*)
	paras = Range[0, 1, 1/(nPts - 1 + 2)] // N;
	(*define the knots for order sd and cpn degrees of freedom*)
	knots = Join[ConstantArray[0., sd], N@Range[0, 1, 1/(cpn - sd)], ConstantArray[1., sd]];

	(*generate the coefficient matrix*)
	coefficientMat = Basis[sd, knots, paras];

	(*maker reg coefficient matrix*)
	coefficientMatDD = ListConvolve[{1, -2, 1}, #] & /@ coefficientMat;
	coefficientMat = Transpose[coefficientMat[[All, 2 ;; -2]]];
	smooth = reg (coefficientMatDD . Transpose[coefficientMatDD]);
	coefficientMatR = Join[coefficientMat, smooth];
	(*output*)
	{coefficientMat, coefficientMatR}
]
...
UnitComp = Compile[{{min, _Real, 0}, {max, _Real, 0}, {x, _Real, 0}}, 
	If[min <= x < max, 1, 0], RuntimeAttributes -> {Listable}, RuntimeOptions -> {"Speed", "WarningMessages" -> False}];

(*b-spline division i*)
DivComp1 = Compile[{{d, _Real, 0}, {u, _Real, 0}, {x, _Real, 1}}, 
	If[d == 0., x, (x - u) / d], RuntimeAttributes -> {Listable}, RuntimeOptions -> {"Speed", "WarningMessages" -> False}];

(*b-spline division i+1*)
DivComp2 = Compile[{{d, _Real, 0}, {u, _Real, 0}, {x, _Real, 1}}, 
	If[d == 0., x, (u - x) / d], RuntimeAttributes -> {Listable}, RuntimeOptions -> {"Speed", "WarningMessages" -> False}];
