...
BlochSeries::usage = 
"BlochSeries[vectorIn, delta_t, freqRange, B1] performs a Bloch simulation of an RF pulse."

Pulses::usage = 
"Pulses[name] gives the pulse shape of some predefined Philips pulse shapes."

GetPulseProfile::usage = 
"GetPulseProfile[excitation, refocus] gives the pulse angle profiles for the excitation and refocusing pulses.
a pulse is defined as {\"name\", flip angle, {G_strength, Dur, BW}}.
GetPulseProfile[{\"name\", flip angle, {G_strength, Dur, BW}}] gives detailed slice profile information of one pulse.

output is {ex_angle_profile, ref_angel_profile, {plots}}.
output for single pulse is {{distance, Mt, Mz, Mx, My, ang, phase}, plots}."

SimulateSliceEPG::usage = 
"SimulateSliceEPG[excitation, refocus, {{t1, t2}, {nEcho, echoSp}, b1}] gives a simulated slice profile and EPG signal plot.
excitation and refocus are generated by GetPulseProfile."

CalculateGFactor::usage = 
"CalculateGFactor[factors, sensitivity, wMat] calculates a gFactor for given sensitivity maps and noise corelation w. given the sense factors which is a list of three integers."

GFactorSimulation::usage = 
"GFactorSimulation[sensitivity, cov, {dir,sense}] calculates the g-factor maps for given sensitivity maps and noise corelation cov in one direction. 
The sense factors are a list of integers in a given direction: \"LR\", \"FH\", or \"AP\".
GFactorSimulation[sensitivity, cov, {dir1,sense1}, {dir2,sense2}] calculates the g-factor maps for given sensitivity maps and noise corelation w in two directions."
...
Using label as plot label and xData as x axis label. tr are the true parameter values."

PlotSimulation::usage = 
"PlotSimulation[pars, xval, true, label, color] plots the pars (output form Parameters). Using label as PlotLabel and xval as x axis thicks.
...
"PlotSimulationAngleHist[pars, label, xData] plots pars (output from AngleParameters)."

PlotSimulationAngle::usage = 
"PlotSimulationAngle[par, xData, label, col] plots pars (output from AngleParameters)."

PlotSimulationVec::usage =
"PlotSimulationVec[tens, xData, label] plots the eigenvectors from simulated tensors."

GESignal::usage = 
"GESignal[ang, {tr, t1}] calculates the gradient echo signal for flip angles ang using tr and t1. 
GESignal[ang, {{tr1, tr2}, t1}] calculates the dual tr gradient echo signal for flip angles ang using tr1, tr2 and t1."
...
"FatFieldStrength is an option for GetPulseProfile. If the value >0 it will calculate the shift of the fat refocusing pulse compared to the fat excitation pulse.
...
CreateDiffData::eig = "eigen system must be 3 eigenvalues {l1,l2,l2} in which cases fixed vectors wil be used 
	or eigenvalues with given vectors {{l1,l2,l3},{e1,e2,e3}} 
	other possibility is 3 eigenvalues with a random, random z or random orthogonal (fixed vectors with random sign)
		{{l1,l2,l3},Random}
		{{l1,l2,l3},RandomZ}
		{{l1,l2,l3},OrtRandom}
	not : `1`"
...
GetPulseProfile[ex_?ListQ, ref_?ListQ, opts : OptionsPattern[]] := Module[{
		exOut, pl1, refOut, pl2, samples, sl1, sl2, shift, step, fatShift
	},

	samples = OptionValue[SliceRangeSamples];
	...
	If[OptionValue[FatFieldStrength] > 0,
		...
		{exOut[[6, samples + 1 ;;]], refOut[[6, samples + 1 ;;]], fatShift, {pl1, pl2}}
		,
		{exOut[[6, samples + 1 ;;]], refOut[[6, samples + 1 ;;]], {pl1, pl2}}
	]
]
...
GetPulseProfileI[{name, flipAngleI, {gradStrengthI, durationI, bandwidthI}}, ops] = Block[{
		gamma, gradStrength, duration, bandwidth, sliceRange, sliceSamp, maxFreq, flipAngle, pos,time,maxTime,
	...
	sliceRange = N@OptionValue[SliceRange]/2 10^-3;(*in mm convert to m, only need to simulate half a slice*)
	sliceSamp = (2*OptionValue[SliceRangeSamples]);(*number of slice samples assume symmetry*)
	...
	plot = GraphicsGrid[Transpose@{
		{
			ListLinePlot[Transpose@{time, power pulse 10^6}, PlotRange -> {{-maxTime,maxTime},({-0.2,.2} + MinMax[pulse]) power 10^6}, PlotLabel -> "RF pulse", opts],
...
SimulateSliceEPG[excitation_, refocus_, {{t1_, t2_}, {nEcho_, echoSp_}, b1_}, OptionsPattern[]] := Module[{
	...
	sig = EPGSignal[{nEcho, echoSp}, {t1, t2}, #, b1] & /@ Transpose[{excitation, refocus}];
	sig = Join[Reverse[sig[[2 ;;]]], sig];
	maxSig = Max[Flatten[sig]];
	sig = sig/maxSig;
	sigT = Mean[sig]/Max[Mean[sig]];

	sigTrue = EPGSignal[{nEcho, echoSp}, {t1, t2}, {First@excitation, First@refocus}, b1]/maxSig;
	...
	fit = If[OptionValue[ReportFits],
		...
		epgT2T = NonLinearEPGFit[{{{nEcho, echoSp}, {1400, 365, 135}, {excitation, refocus}}, {5, 80, 0.3}}, sigT][[1 ;; 4]];
...
CalculateGFactorI[factorsI_, sensitivity_, wMat_, maskI_, OptionsPattern[]] := Block[{
	...
	FOVux,FOVuy,FOVuz, corX, corY,corZ, usf, FOVu,shift,FOVf,r,FOV,
	corsF,corsG,sMat,mat,pMat,gFac,gFactor,mask
	},
	...
	gFactors = Monitor[
		Table[(
			...
			(*get the under sample factors*)
			usVal = Transpose[UsFactor[dim, factor]];
			{FOVux, FOVuy, FOVuz} = usVal[[All, 2]];
			(*get all the indexes for x y and z*)
			{corX, corY, corZ} = Table[
				{usf, FOVu, shift, FOVf, r} = usVal[[c]];
				FOV = dim[[c]];
				(*get the folded x,y and z indexes*)
				Table[Ceiling[If[! (FOV < # <= FOVf), If[# > FOV, # - FOVf, #], FOV] & /@ (cor + FOVu*Range[0, r - 1] + shift)], {cor, FOVu}]
			, {c, 1, 3}];
			(*map the indexes to coordinates*)
			corsF = Flatten[Table[Flatten[Table[{i, j, k}, {i, corX[[x]]}, {j, corY[[y]]}, {k, corZ[[z]]}], 2], {x, FOVux}, {y, FOVuy}, {z, FOVuz}], 2];
			...
			MapThread[(gFactor[[#1[[1]], #1[[2]], #1[[3]]]] = #2) &, {corsF, corsG}, 2];
...
Module[{eig,adc,fa,dataAll,yRange,bins,wBins,sol,fit,x,omega,xi,alpha},
	(
		...
		yRange={{0,3},{0,3},{0,3},{0,3},{0,1}};
		bins=MapThread[{Range[#2[[1]]+.5(#2[[2]]/50),#2[[2]],#2[[2]]/50],BinCounts[#1,{#2[[1]],#2[[2]],#2[[2]]/50}]}&,{dataAll,yRange}];
		wBins=Transpose/@MapThread[{#1[[1]],(#1[[2]]/Length[#3])/(#2[[2]]/100)}&,{bins,yRange,dataAll}];
...
PlotSimulationHist[pars_,label_,xData_,tr_]:=DynamicModule[{yRange,xLabel,exp},
	yRange={{0,3},{0,3},{0,3},{0,3},{0,1}};
	xLabel={lambda[1]<>unit, lambda[2]<>unit, lambda[3]<>unit, "MD"<>unit, "fa [-]"};
	Manipulate[
		If[!ListQ[pars]||!ListQ[yRange],
			Return[],
			exp=GraphicsRow[(Show[
				Histogram[Flatten[pars[[y,1,#]]],{yRange[[#,2]]/50},"ProbabilityDensity",PlotRange->{yRange[[#]],{0,1.1Max[pars[[y,2,#,All,2]]]}},
				...
				ListPlot[pars[[y,2,#]],Joined->True,PlotStyle->{Thick,Black},PlotRange->{yRange[[#]],{0,1.1Max[pars[[y,2,#,All,2]]]}}],
				Plot[pars[[y,3,#]][x],{x,yRange[[#,1]],yRange[[#,2]]},PlotStyle->{Thick,Red},PlotRange->{yRange[[#]],{0,1.1Max[pars[[y,2,#,All,2]]]}}],
...
PlotSimulation[pars_,xval_,tr_,label_,color_,OptionsPattern[]]:= Module[{pl,dat,err,xRange,yRange,yLabel,yTrue,off},

	off=0.025(Max[xval]-Min[xval]);
	xRange={Min[xval]-off,Max[xval]+off};
	yRange=OptionValue[PlotRange];
	yLabel={lambda[1]<>unit, lambda[2]<>unit, lambda[3]<>unit, "MD"<>unit, "fa [-]"};
	pl=(
	yTrue=ConstantArray[tr[[#]],{Length[xval]}];
	dat=pars[[All,4,#,1]];
	err=If[Length[pars[[1,4,#]]]==2,pars[[All,4,#,2]],0];
	ListLinePlot[
		{Transpose[{xval,yTrue}],Transpose[{xval,dat}],Transpose[{xval,dat-err}],Transpose[{xval,dat+err}]},
...
PlotSimulationAngle[par_, xData_, label_, col_, quantileInp_, OptionsPattern[]] := Module[{quant,pars=Transpose[par[[All,4]]],e,pDat,xRange,off},
...
PlotSimulationVec[tens_, xData_, label_, OptionsPattern[]] := Module[{
		eigVec, arrows1, arrows2, arrows3, vp1, vp2, vp3, va1, va2, va3, vv1, vv2, vv3, data, exp, xd
	},

	eigVec = Flatten[EigenvecCalc[#,MonitorCalc->False], 1] & /@ tens;

	eigVec =If[OptionValue[SortVecs],
		Map[(
			{
	...
	Manipulate[
	If[!ListQ[eigVec],Return[],
		data = eigVec[[set]] // Transpose;
		xd = xData[[set]];
	...
	Column[{
		Row[{
			ListLinePlot[{s1, s2a}, PlotRange -> If[! r, Full, {-0, 1}], ImageSize -> 350, PlotStyle -> (Directive[{Thick, #}] & /@ {Black, Red}),
				AxesStyle -> Directive[{Thick, Black}], LabelStyle -> Directive[Bold, 14, Black], PlotLabel -> "Signal as function of flip angle",
				GridLines -> {{fa}, {p1, p2}},
			...
			ListLinePlot[s2a/s1, PlotRange -> {0, 1.1}, ImageSize -> 350, PlotStyle -> Directive[{Black, Thick}], 
				AxesStyle -> Directive[{Thick, Black}], LabelStyle -> Directive[Bold, 14, Black], PlotLabel -> "Signal ratio as function of flip angle",
				GridLines -> {{fa}, {p2/p1}},
