...
RoemerEqualNoise needs the signal and the noise covariance matrix. Can be performed with and without the sense data, without sense data the sensitivity is estimated using the signal and the RSS reconstruction of the signa.
...
ZeropadData::usage = 
"ZeropadData[data] upscales the data resolution by a factor 2 by zeropadding."
...
"CoilWeightedReconCSI[kspace, noise, head] performs reconstruction of raw 3DCSI data. The input kspace, noise and head are obtained using ReadListData.
...
FourierShiftData[data_, shift_] := If[ArrayDepth[data] === 3,
	FourierShiftData[data, shift],
	RotateDimensionsLeft[FourierShiftData[#, shift] & /@ RotateDimensionsRight[data]]
]
...
CovToWeightChol[cov_] := Inverse[ConjugateTranspose[CholeskyDecomposition[cov]]];
...
(* ::Subsubsection::Closed:: *)
(*HamminZeropadDatagFilterData*)


ZeropadData[data_] := Block[{dim},
	dim = Dimensions[data];
