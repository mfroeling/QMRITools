...
Output is {{rawData, noise}, {head, types}}.
ReadListData[file, print] does the same but if print is set False no reporting is done."


MeanType::usage = 
"MeanType[kspace, types, type] calculates the Mean of the kspace data on type, where type is part of types. The kspace and types are generated by ReadListData.
MeanType[{kspace, types}, type] calculates the Mean of the kspace data on type, where type is part of types.
MeanType[kspace, types, {type,..}] calculates the Mean of the kspace data on each of the list type, where type is part of types.
Output is {kspace, types}."

TotalType::usage = 
"TotalType[kspace, types, type] calculates the Total of the kspace data on type, where type is part of types. The kspace and types are generated by ReadListData.
TotalType[{kspace, types}, type] calculates the Total of the kspace data on type, where type is part of types.
TotalType[kspace, types, {type,..}] calculates the Total of the kspace data on each of the list type, where type is part of types.
Output is {kspace, types}."

AverageKspace::usage = 
"AverageKspace[kspace, types, met] calculates the average of the kspace over the type \"N_aver\". The method that can be used
are \"Mean\", \"Total\", \"Hamming\"."
...
"SagittalTranspose[data] makes a transpose of the data of the second level and reverses the slices."
...
"ShiftedFourier[kspace] performs a FourierTransform on the kspace and then shifts the data half the data dimensions."
...
FourierShiftData::usage = 
"FourierShiftData[data, {dz, dy, dx}}] shifts the data with the specified voxels dx, dy and dz.
Output data is complex data."
...
Possible coil combination methods are \"Sum\", \"RootSumSquares\", \"RoemerEqualNoise\", \"RoemerEqualSignal\", \"WSVD\".
RootSumSquares needs the signal. Can be performed with and without the noise covariance matrix
RoemerEqualNoise needs the signal and the noise covariance matrix. Can be performed with and without the sense data, without sense data the sensItivity is estimated using the signal and the RSS reconstruction of the signal.
RoemerEqualSignal needs the signal and the noise covariance matrix and the sense data.
...
MakeSense::usage = 
"MakeSense[coils, cov] makes a sense map for coils. Each coil signal is divided by the RSS reconstruction of the coils."

NoisePrewhitening::usage = 
"NoisePrewhitening[sig, cov] prewhiten the signal sig using the noise covariance matrix cov. The signal and covariance matrix can be any dimension."
...
"HammingFilterData[kspace] applies a Hamming filter to the data. The data is in image space and can be 1D, 2D or 3D."

ZeroPad::usage = 
"ZeroPad[data] up scales the data resolution by a factor 2 by zero padding."

HammingFilterCSI::usage = 
"HammingFilterCSI[kspace] applies a Hamming filter to the k-space data. The data can be can be 1D, 2D or 3D, the spectral dimensions is the last dimensions (x,y,z, spectra)."


DeconvolveCSIdata::usage = 
"DeconvolveCSIdata[spectra] deconvolves the CSI spectra after HammingFilterCSI to revert the blurring of the Hamming filtering.
...
FourierRescaleData[data, factor] rescales the data to factor times the dimensions using zero padding in fourier space."

CoilWeightedReconCSI::usage =
"CoilWeightedReconCSI[kspace, noise, head] performs reconstruction of raw 3D CSI data. The input kspace, noise and head are obtained using ReadListData.
The coil combination Methods can be \"Roemer\" or \"WSVD\"."

CoilWeightedRecon::usage =
"CoilWeightedRecon[kspace, noise, head] performs reconstruction of raw MS2D MRI data. The input kspace, noise and head are obtained using ReadListData.
...
"WienerRegularization is an option for DeconvolveCSIdata. It defines te amount of regularization used in the wiener deconvolution." 
...
CoilSamples::usage = 
"CoilSamples is an option for CoilWeightedReconCSI and specifies how many fud samples are used to calculate the coil sensItivity for Roemer reconstruction."
...
DenoiseCSI::usage = 
"DenoiseCSI is an option for CoilWeightedReconCSI. Specifies if denoiseing needs to be applied. Can be \"Coils\", \"Spectra\" or False."
...
SenseWeight::usage = 
"SenseWeight is an option for MakeSense. Is a integer between 0 and Infinity and define the amount of SOS weighting in the SENSE map."
...
ReadListData[file_,print_]:=Block[{
		...
		typ,pos,dataSplit,indexSplit, echo, scale, locs,cp,cn,cd,p,
	...
	(*get accepted sample data and index*)
	data = scale data;
	...
	(*print output*)
	If[print,
		Print["Data types in data: ", ("typ"/.dataVals[[1]])];
...
AverageKspace[kspace_, met_?StringQ]:=AverageKspace[kspace, {}, met]

AverageKspace[kspace_, types_?ListQ, met_ : "Hamming"] := Block[{kOut, ham},
	kOut = Switch[met,
		"Total" | "Hamming", 
		If[types==={}, TotalAt[kspace, 1], First@TotalType[kspace, types, {"N_aver"}]],
		"Mean", 
		If[types==={}, MeanAt[kspace, 1], First@MeanType[kspace, types, {"N_aver"}]]
	];

	If[met === "Hamming", 
		ham = DivideNoZero[
			MakeHammingFilter[Dimensions[kspace][[4 ;;]]],
			Total[Unitize@kspace[[All, 1, 1]]]
		];
		Map[ham #&, kOut, {2}],
		kOut
	]
...
ShiftedFourier[time_] := FourierShift[Fourier[time, FourierParameters->{0, 1}]];

ShiftedFourier[time_,"Fid"] := FourierShift[Fourier[time, FourierParameters->{0, 1}]];

ShiftedFourier[time_,"Echo"] := FourierShift[Fourier[FourierShift[time], FourierParameters->{0, 1}]];
...
ShiftedInverseFourier[spec_] := InverseFourier[InverseFourierShift[spec],FourierParameters->{0, 1}];

ShiftedInverseFourier[spec_,"Fid"] := InverseFourier[InverseFourierShift[spec],FourierParameters->{0, 1}];

ShiftedInverseFourier[spec_,"Echo"] := FourierShift[InverseFourier[InverseFourierShift[spec],FourierParameters->{0, 1}]];
...
FourierShifted[time_] := Fourier[FourierShift[time], FourierParameters->{0, 1}];
...
InverseFourierShifted[spec_] := InverseFourierShift[InverseFourier[spec, FourierParameters->{0, 1}]];
...
FourierShiftData[data_, shift_] := If[ArrayDepth[data] === 3,
	FourierShiftDataI[data, shift],
	RotateDimensionsLeft[FourierShiftDataI[#, shift] & /@ RotateDimensionsRight[data]]
]


FourierShiftDataI[data_, {dz_, dy_, dx_}] := Block[{kspace, dimZ, dimY, dimX, shiftX, shiftY, shiftZ},
	{dimZ, dimY, dimX} = Dimensions@data;
	kspace = ShiftedFourier[data];

	shiftX = ConstantArray[ShiftRow[dx, dimX], {dimZ, dimY}];
	shiftY = Transpose[ConstantArray[ShiftRow[dy, dimY], {dimZ, dimX}], {1, 3, 2}];
	shiftZ = Transpose[ConstantArray[ShiftRow[dz, dimZ], {dimY, dimX}], {2, 3, 1}];

	ShiftedInverseFourier[shiftX shiftY shiftZ kspace]
]


ShiftRow = Exp[-2. Pi I Range[-Ceiling[#2/2], Floor[#2/2] - 1] (#1/#2)] &;
...
FourierKspace2D[kspace_, head_, filt_:False] := Block[{kSize, data, kspaceF, dim, over, kFull, ksPad, shift, clip, ham},
	...
	(*Reconstruct and Hamming filter the data if needed*)
...
FourierKspaceCSI[kspace_, head_, "3D"]:=Block[{ksPad,dim,imPad,shift,kspaceP,imData},
	...
	(*get the final data dimensions*)
	dim = {"Z-resolution","Y-resolution","X-resolution"}/.head;
	(*pad the kspaces with zeros*)
	kspaceP = ArrayPad[#,Transpose@{ksPad,ksPad},0.+0.I]&/@kspace;
	(*get the image padding and image shift*)
	shift = Total[#]&/@({"Z_range","Y_range","X_range"}/.head);
	(*perform the fourier transform*)
	imData = RotateRight[FourierShift[InverseFourier[FourierShift[#], FourierParameters->{0, 1}]], shift]&/@kspaceP
]
...
Options[NoisePrewhitening] = {
	Method->"PCA"
}

SyntaxInformation[NoisePrewhitening] = {"ArgumentsPattern" -> {_, _, OptionsPattern[]}};

NoisePrewhitening[sig_, cov_, OptionsPattern[]]:= Block[{w, p},
	w = Switch[OptionValue[Method],
		"Choleskey", CovToWeightCholesky[cov],
		"PCA", CovToWeightPCA[cov],
		"ZCA", CovToWeightZCA[cov]
	];
...
CovToWeightCholesky[cov_] := Inverse[ConjugateTranspose[CholeskyDecomposition[cov]]];


CovToWeightPCA[cov_] := Block[{vals, vecs},
	{vals, vecs} = Eigensystem[cov];
	DiagonalMatrix[1/Sqrt[vals]] . Conjugate[vecs]
];


CovToWeightZCA[cov_, eps_ : 1.*^-6] := Module[{vals, vecs, invSqrt},
	{vals, vecs} = Eigensystem[cov];
	Transpose[vecs] . DiagonalMatrix[1./Sqrt[vals + eps]] . Conjugate[vecs]
];
...
CoilCombine[sigI_, opts : OptionsPattern[]] := CoilCombine[sigI, 1, 1, opts]

CoilCombine[sigI_, 1, opts : OptionsPattern[]] := CoilCombine[sigI, 1, 1, opts]

CoilCombine[sigI_, covI_?MatrixQ, opts : OptionsPattern[]] := CoilCombine[sigI, covI, 1, opts]

CoilCombine[sigI_, sen_, opts : OptionsPattern[]] := CoilCombine[sigI, IdentityMatrix[Length@sen], sen, opts]

CoilCombine[sigI_, covI_, sen_, opts : OptionsPattern[]] := Block[{met, weight, sig, sent, cov, white},

	met = OptionValue[Method];
	white = OptionValue[NoisePrewhiten];

	(*pre whiten signal and noise*)
	white = covI =!= 1 && white;
	sig = If[white, NoisePrewhitening[sigI, covI], sigI];
	cov = If[white, IdentityMatrix[Length@covI], covI];

	(*coils are first index but not for WSVD which is only called by CSI recon*)
	sig = RotateDimensionsLeft@If[met === "WSVD", RotateDimensionsLeft[Transpose[sig]], sig];	

	(*Make sensItivity map if needed put n coils as last dimensions sensItivity*)
	sent = If[met=!="WSVD" && (StringTake[met, 6] === "Roemer"), 
		sent = If[sen === 1, MakeSense[sig, SenseSmoothing -> OptionValue[SenseSmoothing]], sen];
		RotateDimensionsLeft[If[white, NoisePrewhitening[sent, covI], sent]]
		,
		sen
	];

	(*perform ND reconstruction for (coils,ND)*)
	If[met =!="Average" && met =!= "RootSumSquares" && cov === 1, Return[$Failed]];
	Switch[met,
		"Average", MeanCombine[sigI],
		"RootSumSquares", If[covI === 1, RSSCombine[sig], RSSCovCombine[sig, cov]],
		"RootSumSquaresSNR", Sqrt[2] Abs@RSSCovCombine[sig, cov],
		"RoemerEqualNoise", RoemerNCombine[sig, sent, cov],
		"RoemerEqualNoiseSNR", Sqrt[2] Abs@RoemerNCombine[sig, sent, cov],
		"RoemerEqualSignal", RoemerSCombine[sig, sent, cov],
		"RoemerEqualSignalSNR", Sqrt[2] Abs@RoemerSCombine[sig, sent, cov],
		"WSVD", WSVDCombine[sig],
		_, Return[$Failed]
	]
]
...
WSVDCombineT[sig_?MatrixQ] := Block[{u, s, v, i, phi, spec},
...
ZeroPad[data_] := Block[{dim},
	dim = Dimensions[data];
	ShiftedInverseFourier[ArrayPad[ShiftedFourier[data], Thread[{Floor[dim/2], Ceiling[dim/2]}]]]
]
...
CoilWeightedReconCSI[kspace_, noise_, head_, sense_, ops:OptionsPattern[]] := Block[{
		fids, spectra, cov, coils, sosCoils, sens, readout, dim, 
		nEnc, met, noCoils, white, denoise, nCoil, normalize, filter
	},
	{readout, white, denoise, nCoil, normalize, filter, met} = OptionValue[{AcquisitionMethod, NoisePrewhiten, 
		DenoiseCSI, CoilSamples, NormalizeOutputSpectra, HammingFilter, Method}];

	nEnc = If[IntegerQ[head], head, "number_of_encoding_dimensions" /. head];
	dim = Switch[nEnc, 3, "2D", 4, "3D"];
	noCoils = ArrayDepth[kspace] =!= nEnc + 1;

	fids = If[noCoils,

		(*no coil combination for 2D or 3D CSI*)
		RotateDimensionsLeft[FourierKspaceCSI[kspace, head, dim]]
		,

		(*Reconstruct fid per coil*)
		...
		Switch[met,
			"WSVD",	CoilCombine[fids, cov, Method -> "WSVD", NoisePrewhiten->False],
			_, RotateDimensionsLeft[CoilCombine[#, cov, sens, Method -> met, 
				NoisePrewhiten->False] & /@ fids]
		]
	];
