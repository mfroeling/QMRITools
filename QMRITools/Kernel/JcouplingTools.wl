...
"SimHamiltonian[sys] simulates the hamiltonian for a given spin system. The spin system is generated by GetSpinSystem.
...
SimRotate::usage = 
"SimRotate[din, ham ,angle] rotates the spin system din given the hamiltonian ham over angle with phase 90 degrees.
SimRotate[din, ham ,angle, phase] rotates the spin system din given the hamiltonian ham over angle with phase.
...
"SequencePulseAcquire[din, ham] performs a pulse acquire experiment of the spin system din given the hamiltonian ham with a 90 Degree pulse.
SequencePulseAcquire[din, ham, b1] performs a pulse acquire experiment of the spin system din given the hamiltonian ham with a 90 Degree pulse and b1.
The output is a new spin system dOut."
...
SequenceTSE::usage =
"SequenceTSE[din ,ham, {te, nEcho}, {ex, ref}] performs a multi echo spin echo experiment with echo time te with nEcho echos of the spin system din given the hamiltonian ham using ex Degree excitation and ref Degree refocus pulses.
SequenceTSE[din ,ham, {te, nEcho}, {ex, ref}, b1] performs a multi echo spin echo experiment with echo time te with nEcho echos of the spin system din given the hamiltonian ham using ex Degree excitation and ref Degree refocus pulses and b1.
...
t2 the time between a 180 degree RF pulse and the following readout (and 2xt1 the time between two consecutive 180 degree RF pulses.
...
ReadoutOutput::usage = 
"ReadoutOutput is an option for SimReadout and SimSignal and values can be \"all\" and \"each\". When set to \"all\" the total signal and signal is given, when set to \"each\" the signal or spectrum for each peak is given separately."

ReadoutPhase::usage = 
"ReadoutPhase is an option for SimReadout and defines the readout phase in degrees."

ReadoutMethod::usage = 
"ReadoutMethod is an option of SimReadout and can be \"Fid\" or \"Echo\". With \"Fid\" it is also possible to define a delay time in ms {\"Fid\", delay}. 
With \"Echo\" it is also possible to define a delay time in ms {\"Echo\", delay} and it than assumes te is half the readout, or a custom te can be defined {\"Echo\", delay, te}."
...
SimHamiltonian[sisI_,OptionsPattern[]]:=Block[{
	sys, sysJ, sysS, scale, sysSi, names, it, name, Hj, Hres, nSpins, nSpins2, nuc,
	identity, zero, set, HbasisA, HbasisB, states, statesI, st, bas, Hix, Hiy, Hiz, 
	di, Hfx, Hfy, Hfz, wIxy, Ixy, Fxy, wFxy, dn, weight, weightI, Hcs, Hjs, Hjw, 
	ham, hamJ, valD, matU, valDJ, matUJ, Hstruct, bField, gyro},

	bField = OptionValue[FieldStrength];
	nuc = OptionValue[SimNucleus];
	gyro = GyromagneticRatio[nuc];

	sys = If[StringQ[sisI],GetSpinSystem[sisI, CenterFrequency -> OptionValue[CenterFrequency]], sisI];
	...
	identity = SparseArray[IdentityMatrix[nSpins2]];
	...
	(*Create All the angular momentum operators Iix, Iiy and Iiz*)
	{Hix,Hiy,Hiz,di} = Transpose[Table[
		st = statesI[[i]];
		bas = HbasisA[[i]];
		{st Abs[bas], st bas I, identity bas, identity bas}
		,{i,1,nSpins}]
	];
	{Hfx,Hfy,Hfz,dn} = Total/@{Hix,Hiy,Hiz,di};

	(*create the readout angular momentum operators*)
	Ixy = (Hix-Hiy I);
	Fxy = (Hfx-Hfy I);(*unweighted versions*)
	wIxy = scale Ixy;
	wFxy = Total[wIxy];(*weighted for spin occurrence*)
...
SimRotate[din_,ham_,angle_,phase_:90]:=Block[{alpha,ph,dinS,Fx,Fy,Fz,Rz,rotate, pMat,rMat, tMat},
	...
	tMat=pMat . rMat . ConjugateTranspose[pMat];(*define rot matrix*)
	Chop[tMat . din . ConjugateTranspose[tMat]](*predef matrix prevent extra comp*)
];
...
SequenceSteam[din_,ham_,{te_,tm_}]:=Block[{d,tau},
	...
	d=(2/Pi)SimAddPhase[d,ham,j];(*de-phase the transverse signal*)
	...
	d=(Pi/4)SimAddPhase[d,ham,j];(*de-phase the transverse signal*)
	d=SimEvolve[d,ham,tau];(*evolve by tau*)
	d
	,{j,0,270,90}]
]
