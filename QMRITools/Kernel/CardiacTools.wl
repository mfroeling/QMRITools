...
Options[MaskToLines] = {LineThreshold -> .3};

MaskToLines[mask_, vox_, opts : OptionsPattern[]] := Block[{wall, off
	}, wall = CalculateWallMap[mask, vox, MaskWallMap -> False, ShowPlot ->
	 False][[1]]; off = CentralAxes[mask, vox, ShowPlot -> False][[1]]; MaskToLines[
	mask, wall, off, opts]]

MaskToLines[mask_?ArrayQ, wall_?ArrayQ, cent_?ListQ, OptionsPattern[]] := Block[{
	...
	Table[
		...
		If[
			...
			seg = If[#[[1, 1]] == 2, # /. {2 -> 1, 1 -> 2}, #]&[MorphologicalComponents[
				N[1 - maski]]];
			...
			If[
				...
				(*Print["inner and outer mask"];*)
				    (*there is an inner and outer mask-here lines are formed perpendicular
																																				 to the mid wall derivative*)
				...
				dis = Flatten[(pp = #; EuclideanDistance[pp, #]& /@ Nearest[ptsol,
					 pp, 2])& /@ ptsil];
				k = Floor[.8 Mean[dis]];
				dis = .4 Min[dis];
				(*get the wall derivatives*)
				(*make the derivative funciton-vector pointing perpendicularto the wall at wall mid point*)
				{int1, int2} = ListInterpolation[GaussianFilter[walli, {k, k}, #],
					 InterpolationOrder -> 1]& /@ IdentityMatrix[2];
				...
				If[
					...
					lines = Map[
						(
							...
							While[(1 + step <= y <= dimy - step - 1 && 1 + step <= x <= dimx
								 - step - 1 && val <= 0.), {x, y} = offi + (ii step) vec; val = int[x,
								 y]; ii++;];
							ptsi = offi + ((ii - 1) step) vec;
							 (*Then find the first position for which point is outside the
																																									 																mask again*)
							While[(1 + step <= y <= dimy - step - 1 && 1 + step <= x <= dimx
								 - step - 1 && val > 0.), {x, y} = offi + (ii step) vec; val = int[x,
								 y]; ii++;];
...
FindPoint[pmid_, step_, fun_] := Block[
	{ptar, pnear, ni, nd, n, fnear, v, u, p}
	,
...
SegmentsPerSlice[points_,segmi_]:=SegmentsPerSlice[GetSegmentSlices[points],points,segmi,True]
SegmentsPerSlice[points_,segmi_,grp_?BooleanQ]:=SegmentsPerSlice[GetSegmentSlices[points],points,segmi,grp]
SegmentsPerSlice[{start_,ap_,mid_,bas_,end_},points_,segmi_]:=SegmentsPerSlice[{start,ap,mid,bas,end},points,segmi,True]

SegmentsPerSlice[{start_,ap_,mid_,bas_,end_},points_,segmi_,grp_?BooleanQ]:=Block[
{apex,apical,midcavity,basal,segments,sel,location,trans,sl,ind},
...
If[points=!={},
sel=If[#[[1]]===None||#[[2]]===None,0,1]&/@points;
{apex,apical,midcavity,basal}=Select[#,sel[[#]]===1&]&/@{apex,apical,midcavity,basal};
];

(*see how to devide each slice*)
segments=If[MemberQ[{1,2,4,6,8,"AHA","AHA+"},segmi],
segments=Switch[segmi,
"AHA",Thread[{apex,apical,midcavity,basal,_}->{1,4,6,6,0}],
"AHA+",Thread[{apex,apical,midcavity,basal,_}->{2,6,8,8,0}],
_,DeleteCases[Thread[{apex,apical,midcavity,basal,_}->{segmi,segmi,segmi,segmi,0}],{}->_]
];
If[grp||StringQ[segmi],segments,Flatten[Thread/@segments]]
,
$Failed
]
]
...
Graphics[],
Show[If[Length@slines=!=1,Graphics[{Thickness[.025],Darker@Gray,Line[RevShift/@#&/@slines[[All,1]]]}],Graphics[]],
...
Options[CardiacSegment]={StartPoints->"Default",StartSlices->"Default",ReversePoints->True,ReverseDirection->False};
...
DynamicModule[{mask,radiusStart,centers,startPoint,defPoints,revi,flipi,pointsIn,centerpl,slices,sti,api,midi,basi,endi,
carPl,colsOver,car,lm,allpli},
mask=Round[maski];
radiusStart=ConstantArray[radi=Max[Dimensions[mask]]/8,Length[mask]];
...
(*segments*)
segments=SegmentsPerSlice[{start,ap,mid,bas,end},{},segmi,slcgrp];
...
PaneSelector[{False->"",True->Control[{{slcgrp,True,"Use slice grouping:"},{False->"group by slice",True->"group by region"},ControlType->SetterBar}]},Dynamic[NumberQ[segmi]]],
...
out[[sl]]=points[[sl]];
If[flip,out=Reverse/@out];
{revb,flip,(Rev@out),{start,ap,mid,bas,end}}]],
...
(*initialization*)
ControlPlacement->Left,SynchronousUpdating->True,SynchronousInitialization->False]
](*close dialog input*)
...
VecAngleC=Compile[{{pnt,_Real,2},{cent,_Real,1},{num,_Integer,0},{rev,_Real,0}},Block[{x1,x2,y1,y2,ang1,ang2,angs,angs2,angsout,diff,dsep,dwall},
