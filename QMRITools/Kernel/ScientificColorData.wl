...
AddScientificColors[dir_]:=Block[{
		...
		nameDef,groupDef,newGroupsPattern,newSchemeNames,newSchemes,scientificColorMapsGroups,gr,cacheFile
	},
	...
	If[Quiet[Head[ColorData["ScientificColorMapsGroups"]]===ColorData]&&DirectoryQ[dir],
		...
		cacheFile = FileNameJoin[{DirectoryName[dir], "allDef.wxf"}];

		If[FileExistsQ[cacheFile],
			(*allDef was cached on a previous run, skip re-importing every gradient .txt file*)
			allDef = Import[cacheFile, "WXF"],
		...
		Table[ColorData[gr] = Sort[Pick[nameDef, MemberQ[#, gr]& /@ allDef[[All, 2]], True]]
			, {gr, Prepend[scientificColorMapsGroups, "ScientificColorMaps"]}];
		Protect[ColorData];
	]
]
