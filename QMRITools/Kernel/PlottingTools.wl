...
PlotIVIM::usage = 
"PlotIVIM[vals, data, bvals] plots the results of the IVIM fits from IVIMCalc."
...
$plotOptions = {
	...
	FrameStyle -> Directive[Thick, LightDarkV[]], LabelStyle -> Directive[LightDarkV[], Bold, 14]
};
...
LabLeg[im_, {fnts_,legs_}, label_, {legend_, color_, {min_,max_}}] := Block[{lab, bar},	
	...
	bar = If[legend,
		BarLegend[{(ColSel @@ color)[(# - min)/(max - min)] &, {min, max}}, 
			LabelStyle -> Directive[Bold, FontFamily -> "Helvetica", fnts, LightDarkV[]],
...
MakeSliceImages[selData_, {selMask_, vals_?ListQ}, vox:{_,_,_}, OptionsPattern[]]:=Block[{
	...
	Table[
		...
		bar = BarLegend[{colF/@Range[0, 1, .01], ran}, LabelStyle->Directive[{LightDarkV[], Bold, 12}]];
...
Options[PlotIVIM] = {Method -> "", PlotColor -> {Red, Green, Blue, LightDarkV[]}, NormalizeIVIM->"Fit",PlotRange->"Auto",ImageSize->400}
...
cols = If[Length[cols] != 4, {Red,Darker@ Green, Blue, LightDarkV[]}, cols];
...
lstyle = Directive[LightDarkV[], Bold, If[OptionValue[ImageSize]<=200,Small,Medium], FontFamily -> "Helvetica"];
