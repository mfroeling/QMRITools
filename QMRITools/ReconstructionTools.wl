...
"MakeHammingFilter[head] makes a 3D HammingKernel for filtering k-space in 3D CSI data. Where head is generated by ReadListData.
...
"FourierKspaceCSI[kspace,head] apllies a Hammingfilter to the k-space data. Where kspace and head are generated by ReadListData.
FourierKspaceCSI[kspace,head] apllies a Hammingfilter to the data."

HammingFilterData::usage = 
"FourierKspaceData[kspace,head] apllies a Hammingfilter to the k-space data. Where kspace and head are generated by ReadListData.
FourierKspaceData[kspace,head] apllies a Hammingfilter to the data."
...
(*needs 1D and 2D cases*)
MakeHammingFilter[{zi_?NumberQ,yi_?NumberQ,xi_?NumberQ}]:=MakeHammingFilterI[Transpose[{-Floor[{xi,yi,zi}/2],Ceiling[{xi,yi,zi}/2]-1}]];

MakeHammingFilter[head_]:={"kx_range","ky_range","kz_range"}/.head;


MakeHammingFilterI[{{xs_,xe_},{ys_,ye_},{zs_,ze_}}]:=Block[{xm,ym,zm,hx,hy,hz},
	(*get ranges*)
	{xm,ym,zm}=Max[Abs[#]]&/@{{xs,xe},{ys,ye},{zs,ze}};
	(*make filter*)
	Table[(0.54+0.46Cos[(Pi x)/xm])(0.54+0.46Cos[(Pi y)/ym])(0.54+0.46Cos[(Pi z)/zm]),{z,zs,ze},{y,ys,ye},{x,xs,xe}]
]
